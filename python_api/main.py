...
def getUsers(db: Session = Depends(getDB)):
    ...
    return [{"username": u.username, "password": u.hashed_password, "roles": u.roles} for u in users]
...
@app.post("/movies")
def create_movie(data: CreateMovieData, db: Session = Depends(getDB), user_data: dict=Depends(verifyToken)):
    if user_data.get("roles") != "ADMIN":
        raise HTTPException(status_code=403, detail="Not enough permissions")
    tittle = data.title
    genres = data.genres
...
def get_movie(movie_id: int, db: Session = Depends(getDB), user_data: dict=Depends(verifyToken)):
...
def update_movie(movie_id: int, data: CreateMovieData, db: Session = Depends(getDB), user_data: dict=Depends(verifyToken)):
    if user_data.get("roles") != "ADMIN":
        raise HTTPException(status_code=403, detail="Not enough permissions")
    movie = db.query(Movie).filter(Movie.movieId == movie_id).first()
    if not movie:
...
def delete_movie(movie_id: int, db: Session = Depends(getDB), user_data: dict=Depends(verifyToken)):
    if user_data.get("roles") != "ADMIN":
        raise HTTPException(status_code=403, detail="Not enough permissions")
    movie = db.query(Movie).filter(Movie.movieId == movie_id).first()
    if not movie:
...
def get_link(movie_id: int, db: Session = Depends(getDB), user_data: dict=Depends(verifyToken)):
...
@app.post("/links")
def create_link(data: CreateLinkData, db: Session = Depends(getDB), user_data: dict=Depends(verifyToken)):
    if user_data.get("roles") != "ADMIN":
        raise HTTPException(status_code=403, detail="Not enough permissions")
...
def update_link(movie_id: int, data: UpdateLinkData, db: Session = Depends(getDB), user_data: dict=Depends(verifyToken)):
    if user_data.get("roles") != "ADMIN":
        raise HTTPException(status_code=403, detail="Not enough permissions")
    link = db.query(Link).filter(Link.movieId == movie_id).first()
    if not link:
...
def delete_link(movie_id: int, db: Session = Depends(getDB), user_data: dict=Depends(verifyToken)):
    if user_data.get("roles") != "ADMIN":
        raise HTTPException(status_code=403, detail="Not enough permissions")
    link = db.query(Link).filter(Link.movieId == movie_id).first()
    if not link:
...
def get_rating(rating_id: int, db: Session = Depends(getDB), user_data: dict=Depends(verifyToken)):
...
@app.post("/ratings")
def create_rating(data: CreateRatingData, db: Session = Depends(getDB), user_data: dict=Depends(verifyToken)):
    if user_data.get("roles") != "ADMIN":
        raise HTTPException(status_code=403, detail="Not enough permissions")
    userId = data.userId
    movieId = data.movieId
    ...
    newRating = Rating(userId=userId, movieId=movieId, rating=rating, timestamp=timestamp)
    db.add(newRating)
    db.commit()
...
def update_rating(rating_id: int, data: UpdateRatingData, db: Session = Depends(getDB), user_data: dict=Depends(verifyToken)):
    if user_data.get("roles") != "ADMIN":
        raise HTTPException(status_code=403, detail="Not enough permissions")
    rating = db.query(Rating).filter(Rating.id == rating_id).first()
    if not rating:
...
def delete_rating(rating_id: int, db: Session = Depends(getDB), user_data: dict=Depends(verifyToken)):
    if user_data.get("roles") != "ADMIN":
        raise HTTPException(status_code=403, detail="Not enough permissions")
    rating = db.query(Rating).filter(Rating.id == rating_id).first()
    if not rating:
...
@app.post("/tags")
def create_tag(data: CreateTagData, db: Session = Depends(getDB), user_data: dict=Depends(verifyToken)):
    if user_data.get("roles") != "ADMIN":
        raise HTTPException(status_code=403, detail="Not enough permissions")
    userId = data.userId
    movieId = data.movieId
...
@app.get("/tags/{tag_id}")
def get_tag(tag_id: int, db: Session = Depends(getDB), user_data: dict=Depends(verifyToken)):
...
@app.put("/tags/{tag_id}")
def update_tag(tag_id: int, data: UpdateTagData, db: Session = Depends(getDB), user_data: dict=Depends(verifyToken)):
    if user_data.get("roles") != "ADMIN":
        raise HTTPException(status_code=403, detail="Not enough permissions")
    tag = db.query(Tag).filter(Tag.id == tag_id).first()
    if not tag:
...
def delete_tag(tag_id: int, db: Session = Depends(getDB), user_data: dict=Depends(verifyToken)):
    if user_data.get("roles") != "ADMIN":
        raise HTTPException(status_code=403, detail="Not enough permissions")
    tag = db.query(Tag).filter(Tag.id == tag_id).first()
    if not tag:
